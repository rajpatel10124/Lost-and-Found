...
if (!$tableExists) {
    ...
    $sql = "CREATE TABLE found_items (" . implode(', ', $columnDefinitions) . ")";
    ...
    $result = $conn->query("DESCRIBE found_items");
    $existingColumns = [];
    ...
    foreach ($requiredColumns as $column => $definition) {
        if (!in_array($column, $existingColumns)) {
            $sql = "ALTER TABLE found_items ADD COLUMN $column $definition";
            
            // Special handling for primary key
            if ($column === 'id') {
                $sql = "ALTER TABLE found_items ADD COLUMN id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST";
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            if (in_array('date_found', $existingColumns)) {
                $columns[] = 'date_found';
                $values[] = $date_found;
                $placeholders[] = '?';
            }
            ...
            $sql = "INSERT INTO found_items (" . implode(', ', $columns) . ") VALUES (" . implode(', ', $placeholders) . ")";
...
<head>
    ...
    <title>Report Found Item - Lost and Found Portal</title>
    <style>
        body {
    ...
    <div class="container">
        ...
        <?php if ($success): ?>
            <div class="alert alert-success">
                ...
                <p><a href="view-items.php?type=found">View all found items</a> or <a href="index.php">return to the homepage</a>.</p>
            </div>
        <?php else: ?>
            <?php if (!empty($errors)): ?>
            ...
            <form action="report-found.php" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="date_found" name="date_found" required value="<?php echo isset($_POST['date_found']) ? htmlspecialchars($_POST['date_found']) : ''; ?>">
                </div>
